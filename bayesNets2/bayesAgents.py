...
def constructBayesNet(gameState):
    ...
    for o in obsVars:
        edges.append((FOOD_HOUSE_VAR, o))
        edges.append((GHOST_HOUSE_VAR, o))
        variableDomainsDict[o] = OBS_VALS

    #populate variableDomainsDict
...
def fillObsCPT(bayesNet, gameState):
    ...
    def getHouse(place):
        if place == bottomLeftPos:
            return BOTTOM_LEFT_VAL
        elif place == topLeftPos:
            return TOP_LEFT_VAL
        elif place == bottomRightPos:
            return BOTTOM_RIGHT_VAL
        elif place == topRightPos:
    ...
    for hPos in gameState.getPossibleHouses():
        for oPos in gameState.getHouseWalls(hPos):
            oVar = OBS_VAR_TEMPLATE % oPos
            oFactor = bn.Factor([oVar], [GHOST_HOUSE_VAR,FOOD_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assign in oFactor.getAllPossibleAssignmentDicts():
                pNone = pRed = pBlue = 0
                if assign[FOOD_HOUSE_VAR] == getHouse(hPos):
                    pBlue = 1 - PROB_FOOD_RED
                    pRed = PROB_FOOD_RED
                elif assign[GHOST_HOUSE_VAR] == getHouse(hPos):
                    pRed = PROB_GHOST_RED
                    pBlue = 1 - PROB_GHOST_RED
                else:
                    pNone = 1
                if assign[oVar] == BLUE_OBS_VAL:
                    oFactor.setProbability(assign, pBlue)
                elif assign[oVar] == RED_OBS_VAL:
                    oFactor.setProbability(assign, pRed) 
                else: 
                    oFactor.setProbability(assign, pNone)

            bayesNet.setCPT(oVar, oFactor)
