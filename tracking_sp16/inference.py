...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for oldP in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldP)
            pr = self.beliefs[oldP]
            for p in newPosDist.keys():
                dd[p] = dd[p] + (newPosDist[p] *pr) 
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        while x < self.numParticles:
            p = self.particles[x]
            if p not in temp:
                distribution = self.getPositionDistribution(gameState, p)
                temp[p] = distribution
                self.particles[x] =  distribution.sample()
            else:
                self.particles[x] = temp[p].sample()
            x += 1
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                p = p * self.getObservationProb(observation[i], pacman, particle[i], self.getJailPosition(i))
            dd[particle] = dd[particle] + p
        ...
        else: self.initializeUniformly(gameState)
