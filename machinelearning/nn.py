...
class Graph(object):
    ...
    def get_output(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        value = self.out.get(node)
        if value is None:
            return 1.0

        return np.array(self.out[node])
    ...
    def add(self, node):
        ...
        fwd = node.forward(inputs)
        self.out[node] = fwd 
        acc = np.zeros_like(fwd)
        self.gradient[node] = acc
    ...
    def backprop(self):
        ...
        for node in (self.get_nodes()[::-1]):
                ...
                inputs = self.get_inputs(node)
                val = node.backward(inputs, gradient)
                i = 0
                parents = node.get_parents()
                for parent in parents:
                    value = val[i]
                    self.gradient[parent] += value
                    i +=1 
    ...
    def step(self, step_size):
        ...
        for var in self.variables:
            gradient = self.get_gradient(var)
            var.data = var.data - (gradient * step_size)
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [np.array(gradient), gradient.sum(axis=0)]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        temp = np.subtract(inputs[0],inputs[1])
        count = inputs[0].size
        return [1.0/count *  (gradient * temp), 1./count * (gradient * -temp)]
